...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  try {
    const { data } = await axios.get(`${apiUrl}/anime/ongoing`, {
      params: { page },
      timeout: 30000,
    });

    if (!data?.status) throw new Error(data?.error || "Gagal mengambil data dari server");
    if (!data.results?.length) return m.reply(`> Tidak ada anime ongoing di halaman ${page}.`);

    let text = `> *ANIME ONGOING — HALAMAN ${page}*\n\n`;
...
handler.help = ["ongoing <halaman>"];
handler.tags = ["anime"];
handler.command = /^(animeongoing|ongoing)$/i;
